...
public class DVMDAO {
    ...
    public static void upsertDVM(DVM dvm) {
        try {
            ...
            String checkSql = "SELECT 1 FROM DVM WHERE STUD_ID=?";
            PreparedStatement check = con.prepareStatement(checkSql);
            check.setInt(1, dvm.getStuId());
            ResultSet rs = check.executeQuery();

            if (rs.next()) {
                String sql = "UPDATE DVM SET REPEATPAPER=? WHERE STUD_ID=?";
                PreparedStatement ps = con.prepareStatement(sql);
                ps.setInt(1, dvm.getRepeatPaper());
                ps.setInt(2, dvm.getStuId());
                ps.executeUpdate();
            } else {
                String sql = "INSERT INTO DVM (STUD_ID, REPEATPAPER) VALUES (?, ?)";
                PreparedStatement ps = con.prepareStatement(sql);
